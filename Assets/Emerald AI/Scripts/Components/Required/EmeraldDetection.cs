...
namespace EmeraldAI
{
    ...
    public class EmeraldDetection : MonoBehaviour, IFaction
    {
        ...
        public void SearchForTarget (PickTargetTypes pickTargetType)
        {
            ...
            //Only bypass VisibleTargets if there are no currently visible targets. This can sometimes happen if a target happens to be obstructed right when a search is being made.
            //If a potential target is within the AI's detection range and this AI's and the target's Combat State is active, it is okay to bypass the obstructions when searching for said targets.
            else if (EmeraldComponent.CombatComponent.CombatState && LineOfSightTargets.Count > 0)
            {
                BypassedTargets.Clear();

                //Get all line of sight targets that are active, even if they are obstructed
